...
<div class="row">
    <div class="col-md-12">
        <ul class="d-flex justify-content-evenly align-items-center">
            <a type="button" class="text-decoration-none text-reset" href="home">Tous les produits</a>
            <?php foreach ($categories as $category) : ?>
                <form action="home" method="POST">
                    <input type="hidden" name="category_id" value="<?= $category->id ?? '' ?>">
                    <button class="btn btn-link text-decoration-none text-reset" type="submit"><?= $category->name ?></button>
                </form>
            <?php endforeach; ?>
        </ul>
    </div>
